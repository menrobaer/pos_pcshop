...
class QuotationController extends Controller
{
  ...
  public function actionConvertToInvoice($id)
  {
    ...
    try {
      $invoice = new Invoice();
      $invoice->quotation_id = $model->id;
      $invoice->code = $this->generateInvoiceCode();
      $invoice->serial_code = $this->generateInvoiceSerialCode();
      $invoice->customer_id = $model->customer_id;
      $invoice->date = $model->date;
      $invoice->due_date = $model->due_date;
      $remark = trim((string) $model->remark);
      $invoice->remark = $remark;
      $invoice->delivery_fee = $model->delivery_fee ?? 0;
      $invoice->extra_charge = $model->extra_charge ?? 0;
      $invoice->discount_amount = $model->discount_amount ?? 0;
      $invoice->cost_total = $model->cost_total ?? 0;
      $invoice->sub_total = $model->sub_total ?? 0;
      $invoice->grand_total = $model->grand_total ?? 0;
      $invoice->paid_amount = $model->paid_amount ?? 0;
      ...
      if (!$invoice->save()) {
        $errors = implode(
          '<br>',
          ArrayHelper::getColumn($invoice->getErrors(), 0),
        );
        throw new Exception('Failed to create invoice: ' . $errors);
      }
      ...
      foreach ($model->items as $item) {
        ...
        $invoiceItem->serial = $item->serial;
        $invoiceItem->unit = $item->unit ?: 'unit';
        $invoiceItem->quantity = $item->quantity;
        $description = trim((string) $item->description);
        $invoiceItem->description = $description;
        $invoiceItem->discount_type = $item->discount_type;
        $invoiceItem->discount = $item->discount ?? 0;
        $invoiceItem->cost = $item->cost ?? 0;
        $invoiceItem->price = $item->price ?? 0;

        if (!$invoiceItem->save()) {
          $errors = implode(
            '<br>',
            ArrayHelper::getColumn($invoiceItem->getErrors(), 0),
          );
          throw new Exception('Failed to create invoice item: ' . $errors);
        }
      ...
      try {
        Yii::$app->utils::insertActivityLog([
          'params' => ['id' => $model->id, 'invoice_id' => $invoice->id],
        ]);
      } catch (\Throwable $e) {
        // do not block request on logging failure
      }
