...
<div class="purchase-order-form">
    ...
    <?php
    $formOptions = ['id' => 'purchase-order-form'];
    if ($isDuplicate) {
      $formOptions['action'] = ['purchase-order/create'];
    }
                        ...
                        <?php if (!empty($items)): ?>
                          <?php foreach ($items as $index => $item): ?>
                                <tr data-index="<?= $index ?>">
                                    <td>
                                        <?= Html::dropDownList(
                                          "PurchaseOrderItem[$index][product_id]",
                                          $item->product_id,
                                          [
                                            $item->product_id =>
                                              $item->product_name .
                                        ...
                                        <?= Html::hiddenInput(
                                          "PurchaseOrderItem[$index][product_name]",
                                          $item->product_name,
                                          ['class' => 'product-name'],
                                        ) ?>
                                        <div class="mt-1">
                                            <?= Html::textarea(
                                              "PurchaseOrderItem[$index][description]",
                                              $item->description,
                                              [
                                                'class' =>
                                    ...
                                    <td><?= Html::textInput(
                                      "PurchaseOrderItem[$index][serial]",
                                      $item->serial,
                                      [
                                    ...
                                    <td><?= Html::textInput(
                                      "PurchaseOrderItem[$index][quantity]",
                                      $item->quantity,
                                      [
                                    ...
                                    <td><?= Html::textInput(
                                      "PurchaseOrderItem[$index][full_price]",
                                      $item->full_price,
                                      [
                                    ...
                                    <td>
                                        <div class="input-group">
                                            <?= Html::textInput(
                                              "PurchaseOrderItem[$index][discount]",
                                              $item->discount,
                                              [
                                                'class' =>
                                            ...
                                            <?= Html::hiddenInput(
                                              "PurchaseOrderItem[$index][discount_type]",
                                    ...
                                    <td>
                                        ...
                                        <?= Html::hiddenInput(
                                          "PurchaseOrderItem[$index][unit]",
                                          $item->unit,
                                          ['class' => 'unit'],
                                        ) ?>
                                        <?= Html::hiddenInput(
                                          "PurchaseOrderItem[$index][sku]",
                                          $item->sku,
                                          ['class' => 'sku'],
                                        ) ?>
                                    </td>
...
<?php
$productSearchUrl = Url::to(['purchase-order/product-search']);
$js = <<<JS
...
$('#add-item').on('click', function() {
    var row = `
        <tr data-index="\${itemIndex}">
            <td>
                <select name="PurchaseOrderItem[\${itemIndex}][product_id]" class="form-control product-select select2-ajax">
                    <option value="">Select Product</option>
                </select>
                <input type="hidden" name="PurchaseOrderItem[\${itemIndex}][product_name]" class="product-name">
                <div class="mt-1">
                    <textarea name="PurchaseOrderItem[\${itemIndex}][description]" class="form-control form-control-sm description auto-height" rows="4" placeholder="Description"></textarea>
                </div>
            </td>
            <td><input type="text" name="PurchaseOrderItem[\${itemIndex}][serial]" class="form-control serial"></td>
            <td><input type="number" name="PurchaseOrderItem[\${itemIndex}][cost]" class="form-control cost" step="0.01"></td>
            <td><input type="number" name="PurchaseOrderItem[\${itemIndex}][quantity]" class="form-control qty" value="1"></td>
            <td><input type="number" name="PurchaseOrderItem[\${itemIndex}][full_price]" class="form-control full-price" step="0.01">
                <input type="hidden" name="PurchaseOrderItem[\${itemIndex}][price]" class="price">
            </td>
            <td>
                <div class="input-group">
                    <input type="number" name="PurchaseOrderItem[\${itemIndex}][discount]" class="form-control discount" step="0.01" value="0">
                    <button type="button" class="btn btn-outline-secondary toggle-discount-type">$</button>
                    <input type="hidden" name="PurchaseOrderItem[\${itemIndex}][discount_type]" class="discount-type" value="fixed">
                </div>
            </td>
            <td class="item-total fw-bold fs-6">0.00</td>
            <td>
                <button type="button" class="btn btn-danger btn-sm remove-item"><i class="ri-delete-bin-line"></i></button>
                <input type="hidden" name="PurchaseOrderItem[\${itemIndex}][unit]" class="unit" value="unit">
                <input type="hidden" name="PurchaseOrderItem[\${itemIndex}][sku]" class="sku">
            </td>
        </tr>
    `;
    var \$row = $(row);
...
function calculateTotals() {
    ...
    var deliveryFee = parseFloat($('[name="PurchaseOrder[delivery_fee]"]').val()) || 0;
    var extraCharge = parseFloat($('[name="PurchaseOrder[extra_charge]"]').val()) || 0;
...
$('#purchaseorder-date').on('change', function() {
    var date = $(this).val();
    if (date) {
        var dueDateElem = document.querySelector('#purchaseorder-due_date');
